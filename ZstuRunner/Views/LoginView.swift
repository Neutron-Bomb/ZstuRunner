...
struct LoginView: View {
    ...
    @State var croypto = ""
    @State var execution = ""
    @State var encryptedPasswd = ""
    
    let croyptoReg = /<p id="login-croypto">(.*?)<\/p>/
    let executionReg = /<p id="login-page-flowkey">(.*?)<\/p>/
    
    let baseUrl = URL(string: "https://sso.zstu.edu.cn/login")!
    var queryString: String {
        "username=\(settings.stuID)&type=UsernamePassword&_eventId=submit&geolocation=&execution=\(execution)&captcha_code=&croypto=\(croypto)&password=\(encryptedPasswd)"
    }
    
    let jsvm = JavaScriptCore.JSContext()
    ...
    func login(stuID: String, passwd: String) {
        ...
        let task = URLSession.shared.dataTask(with: baseUrl) { data, response, error in
            // Fetch 'croypto' & 'execution' from HTML tags
            if let error = error {
                print("error: \(error)")
                return
            }
            ...
            if let mimeType = response.mimeType, mimeType == "text/html", let data = data, let dataString = String(data: data, encoding: .utf8) {
                if let croyptoMatch = dataString.firstMatch(of: croyptoReg), let executionMatch = dataString.firstMatch(of: executionReg) {
                    ...
                    encryptedPasswd = (jsvm?.evaluateScript("""
                    ...
                    var croypto = "\(self.croypto)"
                    encryptByDES(password, croypto)
                    """).toString())!
                    // Initialize Login URLRequest
                    var request = URLRequest(url: baseUrl)
                    request.httpMethod = "POST"
                    request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
                    var relQueryString = queryString
                    relQueryString = relQueryString.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!
                    print(relQueryString)
                    relQueryString.replace("+", with: "%2B")
                    request.httpBody = relQueryString.data(using: .utf8)
            ...
            Form {
                Section {
                    HStack {
                        ...
                        TextField("your student ID", text: $settings.stuID)
                    ...
                    HStack {
                        Spacer()
                        NavigationLink("FIND_PASSWORD", destination: {
                            Password(.reset).navigationBarTitleDisplayMode(.inline)
                        }).font(.footnote)
                    }
                }
                .disabled(isLogging)
                ...
                .toolbar {
                    ToolbarItem {
                        ...
                        }, label: { Image(systemName: "screwdriver.fill").foregroundColor(.secondary).imageScale(.medium) })
                    }
                }
                        ...
                        List {
                            Section("DEBUG") {
                                HStack {
                                    Text("croypto")
                                    Spacer()
                                    Text(croypto.isEmpty ? "empty" : croypto).foregroundColor(.secondary)
                                }
                                HStack(alignment: .top) {
                                    Text("execution")
                                    Spacer()
                                    Text(execution.isEmpty ? "empty" : execution).foregroundColor(.secondary)
                                }
                            }
                        }.toolbar {
                            ...
                            ToolbarItem(placement: .navigationBarTrailing) {
                                Button ("CANCEL") { isDebugging.toggle() }
                            }
                        }
                        .navigationTitle("Debug Detail")
...
//                VStack {
//                    Spacer()
//                    NavigationLink("FIND_PASSWORD", destination: { Password(.reset).navigationBarTitleDisplayMode(.inline) }).font(.footnote)
//                }.padding()
//            }
//            VStack {
//                VStack {
//                    TextField("stuID", text: $settings.stuID)
//                        .focused($focus, equals: .stuid)
//                        .onSubmit {
//                            focus = .password
//                        }
...
//                HStack {
//                    Spacer()
//                    NavigationLink(destination: { Password(.reset) }, label: {
//                        Text("重置密码").font(.footnote)
//                    })
...
//                        }.toolbar {
//                            ToolbarItem(placement: .navigationBarLeading) {
//                                Button("EMPTY") { croypto = "" ; execution = "" }
//                            }
//                            ToolbarItem(placement: .navigationBarTrailing) {
//                                Button ("CANCEL") { isDebugging.toggle() }
//                            }
